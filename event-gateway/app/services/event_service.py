...
class EventService:
    ...
    async def submit(self, command: EventCreate):
        logger.info("event received", extra={"eventId": command.event_id, "accountId": command.account_id})
        ...
        if existing:
            if not same_event(existing, command):
                ...
                logger.warning("conflicting event rejected", extra={"eventId": command.event_id, "accountId": command.account_id})
                raise HTTPException(409, detail={"code": "EVENT_ID_CONFLICT", "message": "eventId already exists with different event data"})
            self.audit.record("EVENT_REPLAYED", "REPLAY", event_id=command.event_id, account_id=command.account_id)
            logger.info("event replayed", extra={"eventId": command.event_id, "accountId": command.account_id})
        ...
        try:
            self.repo.add(event)
            self.audit.record("EVENT_STORED", "SUCCESS", event_id=command.event_id, account_id=command.account_id)
            ...
            if existing and same_event(existing, command):
                self.audit.record("EVENT_REPLAYED", "REPLAY", event_id=command.event_id, account_id=command.account_id)
                return existing, False
            self.audit.record("EVENT_CONFLICT_REJECTED", "FAILURE", event_id=command.event_id, account_id=command.account_id)
        ...
        try:
            ...
            if response.status_code >= 400:
                ...
                logger.error("downstream rejected event", extra={"eventId": command.event_id, "accountId": command.account_id, "statusCode": response.status_code})
                raise HTTPException(response.status_code, detail=detail.get("error", detail))
            event.processing_status = "APPLIED"
        except AccountServiceUnavailable:
            event.processing_status = "FAILED"
            event.updated_at = datetime.now(timezone.utc)
            self.repo.save(event)
            self.audit.record("EVENT_PROCESSING_FAILED", "FAILURE", event_id=command.event_id, account_id=command.account_id, details={"reason": "account service unavailable"})
            logger.error("account service unavailable", extra={"eventId": command.event_id, "accountId": command.account_id})
        ...
        event.updated_at = datetime.now(timezone.utc)
        self.repo.save(event)
        self.audit.record("EVENT_APPLIED", "SUCCESS", event_id=command.event_id, account_id=command.account_id)
        logger.info("event applied successfully", extra={"eventId": command.event_id, "accountId": command.account_id})
        return event, True
