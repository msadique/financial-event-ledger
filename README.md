...
Both FastAPI services are also instrumented with OpenTelemetry. The Gateway creates the server span, HTTPX creates a child client span for the Account Service call, and the Account Service continues the same W3C `traceparent` context. Spans are exported over OTLP/HTTP to the OpenTelemetry Collector, batched, and forwarded over OTLP/gRPC to Jaeger.
...
The Collector forwards distributed traces to Jaeger and converts spans into RED metrics (request rate, error rate, and latency) through the `spanmetrics` connector. Prometheus scrapes those metrics, and Jaeger queries Prometheus to populate its **Monitor** tab.

The Gateway response includes `X-OpenTelemetry-Trace-ID`, which can be opened directly in Jaeger at `http://localhost:16686`. The Jaeger Monitor view is available at `http://localhost:16686/monitor`.
...
Collector RED metrics: http://localhost:8889/metrics
```
...
|---|---:|---:|
| Account Service | 25 passed | **100.00%** |
| Event Gateway | 51 passed | **100.00%** |
| **Total** | **76 passed** | **100.00%** |

Both services exceed the configured minimum coverage threshold of 80% and currently have full statement and branch coverage.
...
Start the complete observability stack and verify that one trace spans both services:

```bash
make test-tracing
```

The validation script submits an event, reads the `X-OpenTelemetry-Trace-ID` response header, queries the Jaeger API, and confirms that the same trace contains spans from both `event-gateway` and `account-service`.
...
## Jaeger Monitor / service performance metrics

The Jaeger **Monitor** tab requires span-derived RED metrics in addition to raw traces. The Collector's `spanmetrics` connector generates call-count and duration histogram metrics. Prometheus scrapes them from port `8889`, and Jaeger queries Prometheus.
...
The script generates 60 traced requests over approximately one minute, waits for Collector and Prometheus flush cycles, and verifies all three stages:

1. The Collector exposes span metrics for `event-gateway` and `account-service`.
2. Prometheus scrapes and stores those metrics.
3. Jaeger's metrics API returns data used by the Monitor tab.
...
```bash
curl -s http://localhost:8889/metrics | grep -E 'calls_total|duration_.*_(count|bucket)'
curl -s 'http://localhost:9090/api/v1/query?query=%7B__name__%3D~%22.*calls_total%22%7D'
END_TS="$(($(date +%s) * 1000))"
curl -s -G 'http://localhost:16686/api/metrics/calls' \
  ...
  --data-urlencode "endTs=${END_TS}" \
  --data-urlencode 'lookback=300000' \
  --data-urlencode 'step=5000' \
  --data-urlencode 'ratePer=60000'
```

The automated test spaces 60 requests one second apart and then waits 15 seconds so Prometheus has enough counter samples for Jaeger to calculate rates. Jaeger is pinned to version 2.19.0 in Docker Compose.
...
- OpenTelemetry, Prometheus, and Jaeger guide: `docs/opentelemetry-jaeger.md`
